...
import argparse
import os
from controller.rc_mpc_cplex import MPC
import utils
...
if __name__=="__main__":
    ...
    parser.add_argument('-w', '--weight', type=float, default=2.2, help='Weight in the cost funtion')
    ...
    parser.add_argument('-sh','--shrinkage', type=float, default=0.5, help='range to shrinkage')
    ...
    # for i in range(int(24*1*(3600/CONTROL_INTERVAL))):
        ...
        UpperSetp = np.array(w['UpperSetp[1]'][1:])-273.15-SHRINKAGE
        LowerSetp = np.array(w['LowerSetp[1]'][1:])-273.15+SHRINKAGE
        ...
        u=controller.forecast(current_states,disturbance,price,UpperSetp,LowerSetp)
        # if i%24==0:
        #     print(f'day:{i//24+1}')
    ...
    print(f"sh={SHRINKAGE}, cost_tot: {formatted_cost}, tdis_tot: {formatted_tdis}, runtime: {duration:.2f} seconds")

    if PLOT:
        if TIME_PERIOD=='peak':
            with open(os.path.join(log_dir, f'kpis_16_sh={SHRINKAGE}.json'), 'w') as f:
                json.dump(kpis, f)
            utils.get_and_plot_results(url=url,log_dir=log_dir, cost_tot=cost_tot,tdist_tot=tdist_tot,start_time=16 * 24 * 3600, final_time=(16+14) * 24 * 3600,w=WEIGHT,p=TIME_PERIOD,horizon=CONTROL_HORIZON,interval=CONTROL_INTERVAL,controller='MPC_Cplex',shrinkage=SHRINKAGE)
        else:
            with open(os.path.join(log_dir, f'kpis_108_w={SHRINKAGE}'), 'w') as f:
                json.dump(kpis, f)
            utils.get_and_plot_results(url=url,log_dir=log_dir,  cost_tot=cost_tot,tdist_tot=tdist_tot,start_time=108 * 24 * 3600, final_time=(108 + 14) * 24 * 3600,w=WEIGHT,p=TIME_PERIOD,horizon=CONTROL_HORIZON,interval=CONTROL_INTERVAL,controller='MPC_Cplex',shrinkage=SHRINKAGE)
